...
<script>
    ...
    const calculateOverallScores = () => {
        ...
        document.querySelectorAll('#student-evaluation-table tbody tr').forEach(row => {
            ...
            const reason = document.getElementById('student-reason').value;
            const deductSemesters = parseInt(document.getElementById('student-divisor').value) || 0;

            let overallAverage;
            if (reason === "Not Applicable") {
                // Reason: Not Applicable
                overallAverage = (rating1 + rating2) / 2;
            } else {
                // Calculate with divisor
                overallAverage = (rating1 + rating2) / (8 - deductSemesters);
            }

            // Update row-wise overall average rating
            row.querySelector('input[name="student_overall_average[]"]').value = overallAverage.toFixed(2);

            studentTotal += overallAverage;
            studentCount++;
        });
        ...
        document.querySelectorAll('#supervisor-evaluation-table tbody tr').forEach(row => {
            ...
            const deductSemesters = parseInt(document.getElementById('supervisor-divisor').value) || 0;
            ...
            // Update row-wise overall average rating
            row.querySelector('input[name="supervisor_overall_average[]"]').value = overallAverage.toFixed(2);

            supervisorTotal += overallAverage;
            supervisorCount++;
        });
        ...
        document.getElementById('supervisor-faculty-overall-score').value = (supervisorAverage * 0.24).toFixed(2);
    };
